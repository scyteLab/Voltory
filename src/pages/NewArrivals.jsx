...
export default function NewArrivals() {
  ...
  function setSort(value) {
    const sp = new URLSearchParams(searchParams);
    if (value === "newest") sp.delete("sort"); else sp.set("sort", value);
    sp.delete("page");
    setSearchParams(sp);
  }
  function setPage(n) {
  ...
  const newlyArrivedCount = all.filter((p) => p.badge === "new").length;
  ...
  return (
    <main className="wrap">
      ...
      <section className="lhero lhero--new">
        <div className="lhero__copy">
          <span className="lhero__pill">
            <Sparkles size={13} /> JUST LANDED
          </span>
          <h1>New Arrivals</h1>
          <p>
          ...
          {newlyArrivedCount > 0 && (
            <p className="lhero__sub">
              <b>{newlyArrivedCount}</b> brand-new product{newlyArrivedCount === 1 ? "" : "s"} this week
            </p>
          )}
        </div>
        <img className="lhero__img" src="/banners/hero-new.png" alt="" />
      </section>
        ...
        <section className="cmain">
          ...
          {pageItems.length > 0 ? (
            ...
            <div className="cempty">
              <h3>No new arrivals match these filters</h3>
