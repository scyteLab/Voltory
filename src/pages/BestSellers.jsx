...
export default function BestSellers() {
  ...
  function setSort(value) {
    const sp = new URLSearchParams(searchParams);
    if (value === "bestseller") sp.delete("sort"); else sp.set("sort", value);
    sp.delete("page");
    setSearchParams(sp);
  }
  function setPage(n) {
  ...
  const topRating = all[0]?.rating || 0;
  const topReviews = all[0]?.reviews || 0;
  ...
  return (
    <main className="wrap">
      ...
      <section className="lhero lhero--best">
        <div className="lhero__copy">
          <span className="lhero__pill">
            <Award size={13} /> CUSTOMER FAVOURITES
          </span>
          <h1>Best Sellers</h1>
          <p>
            The products Nigerians are buying most — top-rated and backed by warranty.
          </p>
          {topRating > 0 && (
            <p className="lhero__sub">
              <Star size={13} fill="currentColor" style={{ color: "var(--acc)", verticalAlign: "middle", marginRight: 4 }} />
              Top-rated: <b>{topRating.toFixed(1)} stars</b> across <b>{topReviews}</b> reviews
            </p>
          )}
        </div>
        <img className="lhero__img" src="/banners/hero-best.png" alt="" />
      </section>
        ...
        <section className="cmain">
          <div className="ctoolbar">
            <p className="ctoolbar__count">
              <b>{sorted.length}</b> bestseller{sorted.length === 1 ? "" : "s"}
          ...
          {pageItems.length > 0 ? (
            ...
            <div className="cempty">
              <h3>No bestsellers match these filters</h3>
